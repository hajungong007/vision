...
{
    struct Trainer_experimental
	{
        Trainer_experimental()
        {
            count = 0;
            iter_count = 0;
            novel_received = false;
            db = object_recognition_core::db::ObjectDbParameters(*json_db_).generateDb();
//            *json_db_ = *json_db_in;
//            *object_id_ = *object_id_in;
        }
        static void	declare_io(const tendrils& params, tendrils& inputs, tendrils& outputs)
		{
            ...
            inputs.declare<ecto::pcl::FeatureCloud>("features", "Input features cloud");
            inputs.declare<bool>("last", "True if this is a last frame");
            inputs.declare<bool>("novel", "True if frame is new");
//            outputs.declare(&Trainer_experimental::json_db_, "json_db");
//            outputs.declare(&Trainer_experimental::db_document_, "db_document", "The filled document.");
//            outputs.declare(&Trainer_experimental::object_id_, "object_id", "The object id, to associate this model with.");
		}
        static void declare_params(ecto::tendrils& params)
        {
//            params.declare(&Trainer_experimental::json_db_in, "json_db_in", "The DB parameters", "{}").required(true);
//            params.declare(&Trainer_experimental::object_id_in, "object_id_in", "The object id, to associate this model with.", "{}").required(true);
        }
...
//                float a = 100;
//                db_document.set_attachment<float>("vfh", a);
...
ECTO_CELL(ecto_msd_pcl_exp, ecto_msd_pcl::Trainer_experimental, "Trainer_experimental", "Train the msd_pcl object detection algorithm.")
